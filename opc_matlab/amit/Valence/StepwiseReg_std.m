...
Training_set = U(trainIdx,1:pcN);%molecular(trainIdx,:);

%Training_set = [U(trainIdx,1:pcN);U(leadIdx,1:pcN)];

Train_pred_opt = zeros(338,21);


LB_pred_opt = zeros(69,21);
%LB_pred_1std = zeros(69,21);
LB_set = U(leadIdx,1:pcN);%molecular(leadIdx,:);

FT_pred_opt = zeros(69,21);
%FT_pred_1std = zeros(69,21);
FT_set = U(testIdx,1:pcN);%molecular(testIdx,:);
...
for i=2:runs
    ...
    mdl = stepwiselm(Training_set,K(:,i),'constant','Upper','purequadratic');
    LB_pred_opt(:,i) = max(0,predict(mdl,LB_set));
    Train_pred_opt(:,i) = max(0,predict(mdl,Training_set));
    FT_pred_opt(:,i) = max(0,predict(mdl,FT_set));
    fprintf('\n\t Iteration %d over',i);
...
for oodd = 1:length(leadIdx)
    for prc=1:runs
        LB_pred_opt_adjusted(oodd, prc) = (polyval( P2(:,prc),LB_pred_opt(oodd,prc)))^(1/pw);
    end    
end  

for oodd = 1:length(testIdx)
    for prc=1:runs
        FT_pred_opt_adjusted(oodd, prc) = (polyval( P2(:,prc),FT_pred_opt(oodd,prc)))^(1/pw);
    end    
end  

for oodd = 1:338
    for prc=1:runs
        Train_pred_opt_adjusted(oodd, prc) = (polyval( P2(:,prc),Train_pred_opt(oodd,prc)))^(1/pw);
    end    
end  

%dlmwrite('Subchallenge2LBstepwise100linADJPredsStd.csv',[odorsID(leadIdx) LB_pred_opt_adjusted],'precision',10);
dlmwrite('Subchallenge2FTstepwise100linADJPredsStd.csv',[odorsID(testIdx) FT_pred_opt_adjusted],'precision',10);
dlmwrite('Subchallenge2Trainstepwise100linADJPredsStd.csv',[[odorsID(trainIdx);odorsID(leadIdx)] Train_pred_opt_adjusted],'precision',10);
