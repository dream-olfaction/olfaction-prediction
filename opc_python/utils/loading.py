    ...
    with open(file_path) as f:
        ...
        for line_num,line in enumerate(reader):
            if line_num > 0:
                ...
                line[6:] = [float('NaN') if x=='NaN' else float(x) for x in line[6:]]
...
def format_perceptual_data(perceptual_data, target_dilution=None,
    ...
    p = perceptual_data
    p.rename(columns={'Compound Identifier':'CID','Odor':'Name','subject #':'Subject'},inplace=True)
    ...
    for i,descriptor in enumerate(descriptors):
        dfs[i]['Descriptor'] = descriptor.split('/')[1 if descriptor.startswith('VAL') else 0].title()
...
def get_molecular_data(sources,CIDs):
    ...
    for source in sources:
        ...
        if source == 'gramian':
            ...
            df = pd.read_table('%s/derived/nspdk_r3_d4_unaug_gramian.mtx' % DATA, delimiter=' ', header=None)
    ...
    if kind in ['training','replicated','leaderboard','testset']:
        ...
        for i,row in perceptual_data.iterrows():
            ...
            if replicate or kind != 'replicated':
                CID = row.name[1]
                dilution = row.name[2]
                dilutions = perceptual_data.loc[('Intensity',CID)].index.get_level_values('Dilution')
